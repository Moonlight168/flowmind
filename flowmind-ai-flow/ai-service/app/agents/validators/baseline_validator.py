...
class BaselineValidator:
    ...
    def _validate_flow(self, output: dict, context: ValidatorContext) -> ValidationResult:
        ...
        if deleted and not self._has_delete_intent(context):
            return ValidationResult.from_errors([
                ValidationError("BASE_B001", f"删除了基线节点 {sorted(deleted)}，但用户未要求删除")
            ])

        # edges 保留：基线的 source->target 连线不能被静默删
        baseline_edges = {(e.get("source"), e.get("target")) for e in (context.current_form_data.get("edges") or [])}
        output_edges = {(e.get("source"), e.get("target")) for e in (output.get("edges") or [])}
        deleted_edges = baseline_edges - output_edges
        if deleted_edges and not self._has_delete_intent(context):
            return ValidationResult.from_errors([
                ValidationError("BASE_B002", f"删除了基线连线 {sorted(map(str, deleted_edges))}，但用户未要求删除")
            ])
        return ValidationResult.ok()

    def _validate_form(self, output: dict, context: ValidatorContext) -> ValidationResult:
        baseline_widgets = context.current_form_data.get("widgetList") or []
        if not baseline_widgets:
            return ValidationResult.ok()

        def _names(widgets):
            return {w.get("options", {}).get("name") for w in widgets if w.get("options", {}).get("name")}

        deleted = _names(baseline_widgets) - _names(output.get("widgetList") or [])
        if deleted and not self._has_delete_intent(context):
            return ValidationResult.from_errors([
                ValidationError("BASE_B003", f"删除了基线字段 {sorted(deleted)}，但用户未要求删除")
            ])
        return ValidationResult.ok()

    def _validate_category(self, output: dict, context: ValidatorContext) -> ValidationResult:
        ...
        if baseline_code and output_code and baseline_code != output_code:
            logger.info(f"[baseline] 分类 code 从 '{baseline_code}' 变为 '{output_code}'")
            return ValidationResult.from_errors([
                ValidationError("BASE_B004", f"分类 code 从 '{baseline_code}' 变为 '{output_code}'，但用户未要求修改")
            ])
        return ValidationResult.ok()
