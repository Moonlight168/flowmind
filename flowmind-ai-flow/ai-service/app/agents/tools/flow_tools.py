...
def save_flow_model(
    ...
    Args:
        model_name: 流程模型名称
        model_key: 流程模型标识（唯一键）
        category_code: 所属分类编码
        bpmn_xml: BPMN XML 内容
        description: 流程描述
        auth_token: 用户认证令牌

    Returns:
        保存成功返回模型信息，失败返回 None
    """
    try:
        service = FlowService(auth_token=auth_token)
        ...
        logger.error(f"工具调用失败（save_flow_model）：{e}", exc_info=True)
        return None
...
def update_flow_model(
    ...
    try:
        ...
        logger.error(f"工具调用失败（update_flow_model）：{e}", exc_info=True)
        return None
...
SAVE_FLOW_MODEL_SCHEMA = {
    ...
    "required": ["model_name", "model_key", "category_code", "bpmn_xml"],
}

UPDATE_FLOW_MODEL_SCHEMA = {
    "type": "object",
    "properties": {
        "model_id": {
            "type": "integer",
    ...
    "required": ["model_id", "model_name", "model_key", "category_code", "bpmn_xml"],
}
...
def generate_bpmn_xml(bpmn_structure: dict, category: dict) -> str:
    ...
    支持所有节点类型：USER_TASK、EXCLUSIVE_GATEWAY、PARALLEL_GATEWAY、INTERMEDIATE_THROW_EVENT
    ...
    # 创建连线
    if custom_edges:
        _create_custom_edges(process, ns, custom_edges, node_ids)
    else:
...
def _create_process_element(
    ...
    return etree.SubElement(
        definitions, "{%s}process" % ns["bpmn"], id=process_id, name=process_name,
        isExecutable="true"
    )
    ...
    for node in nodes:
        if node.get("id"):
            ...
            if node_type == "USER_TASK":
                ...
                task_counter += 1
            elif node_type in ("EXCLUSIVE_GATEWAY", "PARALLEL_GATEWAY"):
    ...
    for i, node in enumerate(nodes):
        ...
        if node_type == "USER_TASK":
            ...
            if node.get("candidate_groups"):
                attrs[f"{{{fl}}}candidateGroups"] = node["candidate_groups"]
            if node.get("text"):
                attrs[f"{{{fl}}}text"] = node["text"]
            if node.get("candidate_users"):
                attrs[f"{{{fl}}}candidateUsers"] = node["candidate_users"]
...
def _create_auto_edges(
    ...
    # 创建开始事件
    start_event = etree.SubElement(process, f"{{{bpmn}}}startEvent", id=start_id, name="开始")
    if nodes:
...
def _create_custom_edges(
    ...
    for i, edge in enumerate(edges):
        ...
        source_id = start_id if source == "start" else source
        target_id = end_id if target == "end" else target
        ...
        if edge.get("condition"):
            ...
            condition_elem.set("xsi:type", "bpmn2:tFormalExpression")
        else:
