...
def search_categories(name: str = "") -> list[dict]:
    rows = request_cache.get(
        ...
        lambda: CategoryClient(auth_token=get_auth_token()).search_categories(name),
    )
    return [
        {
...
def search_forms(name: str = "") -> list[dict]:
    rows = request_cache.get(
        ...
        lambda: FormClient(auth_token=get_auth_token()).search_forms(name),
    )
    return [
        {
            "formId": r.get("formId"),
...
def search_roles(name: str = "") -> list[dict]:
    rows = request_cache.get(
        ...
        lambda: RoleClient(auth_token=get_auth_token()).search_roles(name),
    )
    return [
...
def search_flow_models(name: str = "") -> list[dict]:
    rows = request_cache.get(
        ...
        lambda: FlowModelClient(auth_token=get_auth_token()).search_flow_models(name),
    )
    return [
        {
